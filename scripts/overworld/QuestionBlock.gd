...
func _process(_delta):
	if activated:
		...
		if timer < 100 and timer > 0:
			position.y = original_translation.y + sin(timer / 100 * PI) * .25
			for face in faces:
			...
			if timer > 50 and not given_item:
				...
				item.position = original_translation + Vector3(0, .33, 0)
				
				for _i in range(0, coin_count):
					var coin = load("res://scenes/overworld/OverworldCoin.tscn").instantiate()
					$"..".add_child(coin)
					coin.position = original_translation + Vector3(0, .33, 0)
...
func _on_Area_area_entered(area):
	if area == get_node(player).get_node("HammerArea"):
		start_timer()
